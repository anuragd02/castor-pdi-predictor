...
label { color: #6a9a6a !important; font-size: 0.78rem !important; font-weight: 400 !important; }
div[data-testid="stNumberInput"] button { background: #141f14 !important; border-color: #1e3320 !important; color: #4a9f4a !important; }
...
def load_model():
    ...
    if not os.path.exists(model_path):
        st.error("❌ Model file `castor_elastic_net_model.pkl` not found. Place it in the same folder as app.py.")
        st.stop()
...
# ─── Risk data (Low → Moderate → High → Severe) ──────────────────────────────
RISK_DATA = {
    "Low": {
...
with left_col:
    ...
    with c2:
        temp_min = st.number_input("Min Temp (°C)", min_value=0.0, max_value=60.0, value=22.0, step=0.1, format="%.1f")

    st.markdown('<div class="panel-title section-gap">💧 Relative Humidity</div>', unsafe_allow_html=True)
    c3, c4 = st.columns(2)
    with c3:
        rh_max = st.number_input("Max RH (%)", min_value=0.0, max_value=100.0, value=85.0, step=0.1, format="%.1f")
    with c4:
        rh_min = st.number_input("Min RH (%)", min_value=0.0, max_value=100.0, value=45.0, step=0.1, format="%.1f")

    st.markdown('<div class="panel-title section-gap">🌧 Atmospheric & Rainfall</div>', unsafe_allow_html=True)
    c5, c6 = st.columns(2)
    with c5:
        wind_speed = st.number_input("Wind Speed (km/h)", min_value=0.0, max_value=200.0, value=8.0, step=0.1, format="%.1f")
    with c6:
        sunshine = st.number_input("Sunshine (hrs/day)", min_value=0.0, max_value=24.0, value=7.5, step=0.1, format="%.1f")
...
with right_col:
    ...
    if predict_clicked:
        # Compute derived features (mirrors training pipeline)
        ...
        # ── Derived features chips ──
        st.markdown(f"""
        <div class="panel-title" style="margin-top:0.5rem;">⚙️ Derived Features</div>
        <div class="derived-row">
            <div class="dchip"><div class="dv">{temp_diff:.1f}°C</div><div class="dl">Temp Diff</div></div>
            <div class="dchip"><div class="dv">{rh_diff:.1f}%</div><div class="dl">RH Diff</div></div>
            <div class="dchip"><div class="dv">{rain_per_day:.1f}mm</div><div class="dl">Rain/Day</div></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="empty-state">
            ...
            <div class="empty-text">Enter the weather parameters and click <b>Predict Disease Index</b> to see results and advisory.</div>
        </div>
        """, unsafe_allow_html=True)
